...
const omnisharpVersion = workspace
...
const omnisharpRepo: LanguageServerRepository = {
  ...
  channel:
    omnisharpVersion === "latest"
      ? omnisharpVersion
      : `tags/${omnisharpVersion}`,
};

const omnisharpPacks: ILanguageServerPackages = {
  "win-x64": {
    platformPath: "omnisharp-win-x64.zip",
...
export async function activate(context: ExtensionContext) {
  ...
  const omnisharpProvider = new LanguageServerProvider(
    context,
    "OmniSharp",
    omnisharpPacks,
    omnisharpRepo
  );
  const omnisharpExe = useCustomPath
    ? omnisharpCustomPath
    : await omnisharpProvider.getLanguageServer();
  ...
  let cmd_updateomnisharp = commands.registerCommand(
    "omnisharp.downloadLanguageServer",
    async () => {
      if (client.started) {
        await client.stop();
      ...
      if (useCustomPath) {
        window.showMessage(
          `coc-omnisharp: Using custom executable (${omnisharpCustomPath})` +
            " so the downloaded bundle will have no effect",
          "warning"
        );
      }
